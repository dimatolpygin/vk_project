...
func (r *MessageRepo) Get(ctx context.Context, key string) (*Message, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT key, text, image_url, buttons::text, updated_at
		FROM messages WHERE key = $1`, key).
		Scan(&m.Key, &m.Text, &m.ImageURL, &buttonsJSON, &m.UpdatedAt)
...
func (r *MessageRepo) List(ctx context.Context) ([]*Message, error) {
	rows, err := r.db.Query(ctx, `SELECT key, text, image_url, buttons::text, updated_at FROM messages ORDER BY key`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		m := &Message{}
		var buttonsJSON *[]byte
		if err := rows.Scan(&m.Key, &m.Text, &m.ImageURL, &buttonsJSON, &m.UpdatedAt); err != nil {
			return nil, err
		}
		if buttonsJSON != nil {
			_ = json.Unmarshal(*buttonsJSON, &m.Buttons)
...
func (r *MessageRepo) Upsert(ctx context.Context, key, text string, imageURL *string, buttons []Button) error {
	...
	_, err = r.db.Exec(ctx, `
		...
		VALUES ($1, $2, $3, $4, now())
		ON CONFLICT (key) DO UPDATE SET text=$2, image_url=$3, buttons=$4, updated_at=now()`,
