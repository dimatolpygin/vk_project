...
func HandleEditResultStart(ctx context.Context, fc *Context, d *Deps) {
	...
	_ = d.Sender.SendText(ctx, fc.VkID, "✏️ Опиши, что нужно изменить на фото:", KbBack())
}
...
func HandleResultEditPrompt(ctx context.Context, fc *Context, d *Deps) {
	...
	photoURL := fc.State.PhotoURL
	if photoURL == "" {
		_ = d.Sender.SendText(ctx, fc.VkID, "❌ Фото не найдено. Начни новую генерацию.", KbMainMenu())
		return
	}
	if !fc.User.HasGens() {
		_ = d.Sender.SendMsg(ctx, fc.VkID, "no_gens_left", KbBack())
